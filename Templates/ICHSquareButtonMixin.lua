...
function ICHSquareButtonMixin:OnEnter()
    ...
    if self.action == "HELP" then
        ...
        GameTooltip:AddLine("- "..L["Click the star icon on any item to mark or unmark it as a favorite. Favorited items are always pinned to the top of the list."], 1, 1, 1)
        GameTooltip:AddLine("- "..L["Click a column header to sort the list by that column. Click again to reverse the order, or a third time to clear the sort."], 1, 1, 1)
        GameTooltip:AddLine("- "..L["Click on a collectible's icon to preview it in a dressing room window."], 1, 1, 1)
        GameTooltip:AddLine("- "..L["Use the checkboxes at the bottom of the window to control which items appear in the list."], 1, 1, 1)
        GameTooltip:AddLine("- "..L["Enable the mini-window in settings or with /ich mini to see available collectibles while inside an instance."], 1, 1, 1)
    elseif self.action == "SEARCH" then
        ...
        GameTooltip:AddLine("- "..L["dungeon will show all collectibles that can be collected from a dungeon"], 1, 1, 1)
        ...
        GameTooltip:AddLine("- "..L["lich king will show all collectibles dropped by The Lich King"], 1, 1, 1)
        GameTooltip:AddLine("- "..L["northrend will show all collectibles from instances in Northrend"], 1, 1, 1)
        GameTooltip:AddLine("- "..L["Other expansion abbreviations are also supported: tbc, cata, mop, wod, bfa, sl, df, tww"], 1, 1, 1)
        GameTooltip:AddLine("- "..L["wrath|cata will show all collectibles from Wrath of the Lich King or Cataclysm"], 1, 1, 1)
        GameTooltip:AddLine("- "..L["dungeon !heroic will show all dungeon collectibles not available on Heroic difficulty"], 1, 1, 1)
        GameTooltip:AddLine("- "..L["Adjacent terms are combined with AND by default. Use parentheses to group terms, e.g. (wrath|cata) !heroic"], 1, 1, 1)
    end
    GameTooltip:Show()
end
